...
class Jogo():
    ...
    def update(self, action, clock):
        if sapo.estado == True:
            sapo.update()
        if gripe.estado ==True:
            gripe.update()
            tiro.colisao_tiro()
        if porta.animar == True:
            porta.update()
        if chave.estado == True:
            chave.update()
        if laranja.estado == True:
            laranja.update()
        if tiro.estado == True:
            tiro.update()
            tiro.colisao_tiro()
    ...
    def movimento_camera(self, true_scroll):
        ...
        for linha in self.mapa:  # Para linha em mapa
            ...
            for bloco in linha:  # Para bloco em linha
                if bloco == '1':
                    screen.blit(self.img_dna_chao, (self.x * 50 - self.true_scroll[0], self.y * 25 - self.true_scroll
                        [1]))  # Print bloco em relacao ao movimento do player
                if bloco == '2':
                    screen.blit(self.img_andar, (self.x * 50 - self.true_scroll[0], self.y * 25 - self.true_scroll
                        [1]))  # Print bloco em relacao ao movimento do player
                if bloco == '3':
                    screen.blit(self.img_pular, (self.x * 50 - self.true_scroll[0], self.y * 25 - self.true_scroll
                        [1]))  # Print bloco em relacao ao movimento do player
                if bloco == '4':
                    screen.blit(self.img_socar, (self.x * 50 - self.true_scroll[0], self.y * 25 - self.true_scroll
                        [1]))  # Print bloco em relacao ao movimento do player
                if bloco != '0':
                    self.blocos_rects.append \
                        (pygame.Rect(self.x * 50, self.y * 25, 50, 25))  # Adicionar bloco sem conteudo na lista
                self.x += 1  # Bloco + 1
            self.y += 1  # Linha + 1
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        for i in range(1, 9):  # Pegar 8 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'GB' + str(i) + '.png'))  # Pegar frames andar+i da pasta images
    ...
    def colisao_chave(self):
        if chave.estado:
            self.teste_colisao_chave = pygame.sprite.collide_mask(self, chave)  # Variavel que verifica se colisao é true

            if self.teste_colisao_chave:
                    print('pegou chave')
                    chave.var_chave = True
                    chave.kill()

    #Colisao porta e leo
    def colisao_porta(self):
        if porta.estado:
            self.teste_colisao_porta = pygame.sprite.collide_mask(self, porta)  # Variavel que verifica se colisao é true

            if self.teste_colisao_porta:
                if chave.var_chave:
                    print('colidiu porta')
                    porta.animar = True

    #Colisao laranja e leo
    def colisao_laranja(self):
        if laranja.estado:
            self.teste_colisao_laranja = pygame.sprite.collide_mask(self, laranja)  # Variavel que verifica se colisao é true

            if self.teste_colisao_laranja:
                    print('colidiu laranja')
                    laranja.var_laranja = True
                    laranja.kill()


    def colisao_gripe(self):
        if gripe.estado:
            self.teste_colisao_gripe = pygame.sprite.collide_mask(self, gripe)  # Variavel que verifica se colisao é true

            # Teste colisao do soco
            if action == 'soco':  # Se a imagem do soco colidir
                if self.teste_colisao_gripe:
                    print('gameover')
                    gripe.kill()
            else:  # Se qualquer outra imagem colidir
                if self.teste_colisao_gripe:
                        print('colidiu gripe')  # pra gente ver q perdeu tres vidas pq nn sei oq vai acontecer qnd ele morre
        ...
        for e in eventos:
            if e.type == pygame.KEYDOWN:
                ...
                if e.key == pygame.K_SPACE:
                    if laranja.var_laranja == False:
                        action = "soco"
                    if laranja.var_laranja == True:
                        action = "andar"
                        tiro.rect.x = player.rect.right
...
class Sapo(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(1, 6):  # Pegar 5 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'sapinho' + str(i) + '.png'))  # Pegar frames sapinho+i da pasta images
...
class Chave(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.frame = 0
        self.var_chave = False
        self.img_chave = []
        for i in range(1, 4):  # Pegar 5 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'chave' + str(i) + '.png'))  # Pegar frames sapinho+i da pasta images
            self.img = pygame.transform.scale(self.img, (36, 20))  # Redimensionar imagem para 60x60
            self.img_chave.append(self.img)  # Adicionar imagens na lista para o sprite do sapo

        self.image = self.img_chave[0]
        self.rect = self.image.get_rect()  # Pegar rect do sapo
        self.rect.x = 2755  # Posicao inicial do sapo em X
        self.rect.y = 290  # Posicao inicial do sapo em Y


    def kill(self):
        self.estado = False
        for i in self.img_chave:
            self.img_chave.remove(i)


    def update(self):
        if self.estado:
            self.frame = self.frame + 0.2
            if self.frame >= len(self.img_chave):
                self.frame = 0
                self.image = self.img_chave[int(self.frame)]
...
class Porta(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        for i in range(1, 5):
            self.img = pygame.image.load \
                (os.path.join('images', 'porta' + str(i) + '.png'))
        ...
        self.rect.x = 3500  # Posicao inicial
        self.rect.y = 110
    ...
    def update(self):
        ...
        if self.estado:
            self.frame = self.frame + 0.1
            if self.frame >= len(self.img_porta):
                self.frame = 0
            self.image = self.img_porta[3]
...
class Laranja(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(1, 9):  # Pegar 5 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'laranja' + str(i) + '.png'))  # Pegar frames sapinho+i da pasta images
        ...
        self.rect = self.image.get_rect()  # Pegar rect do sapo
        self.rect.x = 3750 # Posicao inicial do sapo em X
        self.rect.y = 270  # Posicao inicial do sapo em Y]

    def kill(self):
        self.estado = False
        for i in self.img_laranja:  # Pegar 5 imagens
            self.img_laranja.remove(i)
...
class Tiro(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(1, 11):  # Pegar 5 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'fogo' + str(i) + '.png'))  # Pegar frames sapinho+i da pasta images
        ...
        self.posicao = player.rect.right
    ...
    def update(self):
            if self.estado:
                ...
                laranja.var_laranja = False

    def kill(self):
        self.estado = False
        for i in self.img_tiro:  # Pegar 5 imagens
            self.img_tiro.remove(i)

    def colisao_tiro(self):
        if gripe.estado:
            self.teste_colisao_tiro = pygame.sprite.collide_mask(self,gripe)  # Variavel que verifica se colisao é true
...
class Gripe(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(1, 5):  # Pegar 5 imagens
            self.img = pygame.image.load \
                (os.path.join('images', 'gripe' + str(i) + '.png'))  # Pegar frames sapinho+i da pasta images
        ...
        self.rect = self.image.get_rect()  # Pegar rect do sapo
        self.rect.x = 3800  # Posicao inicial do sapo em X
        self.rect.y = 250  # Posicao inicial do sapo em Y
    ...
    def kill(self):
        self.estado = False
        for i in self.img_gripe:  # Pegar 5 imagens
            self.img_gripe.remove(i)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        (player.rect.x - int_scroll[0], player.rect.y - int_scroll[1]))  # Atualizar a tela com o movimento da camera
        if chave.estado:
            screen.blit(chave.image,(chave.rect.x - int_scroll[0], chave.rect.y - int_scroll[1]))
        if laranja.estado:
            screen.blit(laranja.image,(laranja.rect.x - int_scroll[0], laranja.rect.y - int_scroll[1]))
        if porta.estado:
            screen.blit(porta.image,(porta.rect.x - int_scroll[0], porta.rect.y - int_scroll[1]))
        if gripe.estado:
            screen.blit(gripe.image,(gripe.rect.x - int_scroll[0], gripe.rect.y - int_scroll[1]))
        if tiro.estado == True:
            screen.blit(tiro.image,(tiro.rect.x - int_scroll[0], tiro.rect.y - int_scroll[1]))
